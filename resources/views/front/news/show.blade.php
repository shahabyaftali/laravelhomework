...
@section('title', $news->title . ' | ' . __('text.awju'))
@section('content')
...
echo asset("storage/images/news/" . $news->image)?>')"></div>
    <div class="background-img background-img--darkened hidden-md hidden-lg"
        style="background-image: url('<?php echo asset("storage/images/news/" . $news->image)?>')"></div>
            ...
            <div class="container">
                <div class="single-header__inner inverse-text js-overlay-bg-sub-area">
                    <div class="background-img background-img--dimmed blurred-more hidden-xs hidden-sm js-overlay-bg-sub"
                        style="background-image: url('<?php echo asset("storage/images/news/" . $news->image)?>')">
                    </div>
                    <div class="single-header__content">
                        <h1 class="entry-title entry-title--lg">{{$news->title}}</h1>
                        <div class="entry-meta af-rtl af-fs"><time class="entry-date published"
                                datetime="{{$news->created_at}}" itemprop="datePublished"
                                title="{{$news->created_at}}"><i
                                    class="mdicon mdicon-schedule"></i>{{$news->created_at->diffForhumans()}}</time>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="theiaStickySidebar"
                    ...
                    <div class="mnmd-widget-indexed-posts-c mnmd-widget widget">
                        <div class="widget__title">
                            <h4 class="widget__title-text af-fs"><span class="first-word">
                              {{__('text.other_news')}}
                              </span>
                            </h4>
                            ...
                            <li>
                                <article class="post post--overlay post--overlay-bottom cat-4">
                                    <div class="background-img background-img--darkened"
                                        style="background-image: url('<?php echo asset('storage/images/news/' . $other->image);?>')"></div>
                                    ...
                                    </div><a href="{{route('news.show', $other)}}" class="link-overlay"></a>
                                </article>
                            </li>
                            ...
                            <li>
                                <article class="post cat-5">
                                    <div class="media">
                                        ...
                                        <div class="media-body media-middle">
                                            <h3 class="post__title typescale-0"><a href="{{route('news.show', $other)}}">{{$other->title}}</a></h3>
                                        </div>
                                    </div>
                                </article>
                            </li>
